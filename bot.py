...
async def ver_tiempos(interaction: discord.Interaction):
    ...
    embeds = create_paginated_embeds(user_data_list, "📊 Todos los Tiempos", 0x3498db)
...
async def paga_alto(interaction: discord.Interaction):
    ...
    # Recopilar usuarios con rol Alto
    alto_users = []
    total_credits_to_pay = 0
        ...
        try:
            ...
            if user:
                ...
                if member:
                    user_role = get_user_role(member)
                    if user_role == 'alto':
                        # Obtener créditos totales guardados
                        saved_credits = get_user_saved_credits(int(user_id))
                        total_credits_to_pay += saved_credits
                        ...
                        alto_users.append({
                            'name': f"{status} {user.display_name}",
                            'value': f"**Tiempo:** {hours}h {minutes}m\n**Créditos totales:** {saved_credits}"
                        })
        except:
            continue
    ...
    if not alto_users:
        embed = discord.Embed(
            ...
            description="No hay usuarios con rol Alto registrados.",
            color=0xE74C3C
        )
        await interaction.response.send_message(embed=embed)
        return

    # Ordenar usuarios por créditos (mayor a menor)
    alto_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(alto_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Alto",
            description=f"**{len(alto_users)} usuarios con rol Alto**",
            color=0xE74C3C
        )
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(alto_users)}**",
            inline=False
        )
        ...
        embeds = create_paginated_embeds(alto_users, "💰 Pagos - Rol Alto", 0xE74C3C, items_per_page=8)
        
        # Agregar resumen al último embed
        embeds[-1].add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(alto_users)}**",
            inline=False
...
async def paga_recluta(interaction: discord.Interaction):
    ...
    # Recopilar usuarios con rol Recluta
    recluta_users = []
    total_credits_to_pay = 0
    ...
    if not recluta_users:
        embed = discord.Embed(
            title="💰 Pagos - Rol Recluta",
            description="No hay usuarios con rol Recluta registrados.",
            color=0x95A5A6
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # Ordenar usuarios por créditos (mayor a menor)
    recluta_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(recluta_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Recluta",
            description=f"**{len(recluta_users)} usuarios con rol Recluta**",
            color=0x95A5A6
        )
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(recluta_users)}**",
            inline=False
        )
        ...
        # Paginación para muchos usuarios
        embeds = create_paginated_embeds(recluta_users, "💰 Pagos - Rol Recluta", 0x95A5A6, items_per_page=8)
...
async def paga_gold(interaction: discord.Interaction):
    ...
    # Recopilar usuarios con rol Gold
    gold_users = []
    total_credits_to_pay = 0
    ...
    if not gold_users:
        embed = discord.Embed(
            ...
            description="No hay usuarios con rol Gold registrados.",
            color=0xFFD700
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # Ordenar usuarios por créditos (mayor a menor)
    gold_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(gold_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Gold",
            description=f"**{len(gold_users)} usuarios con rol Gold**",
            color=0xFFD700
        )
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(gold_users)}**",
            inline=False
        )
        ...
        # Paginación para muchos usuarios
        embeds = create_paginated_embeds(gold_users, "💰 Pagos - Rol Gold", 0xFFD700, items_per_page=8)
...
@bot.tree.command(name="paga_supervisor", description="Ver créditos de usuarios con rol Supervisor")
async def paga_supervisor(interaction: discord.Interaction):

    all_times = tracker.get_all_user_times()

    if not all_times:
        embed = discord.Embed(
            title="💰 Pagos Supervisor",
            description="No hay usuarios con tiempo registrado.",
            color=0x9B59B6
        )
        await interaction.response.send_message(embed=embed)
    ...
    # Recopilar usuarios con rol Supervisor
    supervisor_users = []
    total_credits_to_pay = 0
    ...
    if not supervisor_users:
        embed = discord.Embed(
            title="💰 Pagos - Rol Supervisor",
            description="No hay usuarios con rol Supervisor registrados.",
            color=0x9B59B6
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # Ordenar usuarios por créditos (mayor a menor)
    supervisor_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(supervisor_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Supervisor",
            description=f"**{len(supervisor_users)} usuarios con rol Supervisor**",
            color=0x9B59B6
        )
        
        for user_data in supervisor_users:
            embed.add_field(
                name=user_data['name'],
                value=user_data['value'],
                inline=True
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(supervisor_users)}**",
            inline=False
        )
        ...
        # Paginación para muchos usuarios
        embeds = create_paginated_embeds(supervisor_users, "💰 Pagos - Rol Supervisor", 0x9B59B6, items_per_page=8)
...
@bot.tree.command(name="paga_silver", description="Ver créditos de usuarios con rol Silver")
async def paga_silver(interaction: discord.Interaction):
    ...
    # Recopilar usuarios con rol Silver
    silver_users = []
    total_credits_to_pay = 0
    ...
    if not silver_users:
        embed = discord.Embed(
            title="💰 Pagos - Rol Silver",
            description="No hay usuarios con rol Silver registrados.",
            color=0xC0C0C0
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # Ordenar usuarios por créditos (mayor a menor)
    silver_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(silver_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Silver",
            description=f"**{len(silver_users)} usuarios con rol Silver**",
            color=0xC0C0C0
        )
        
        for user_data in silver_users:
            embed.add_field(
                name=user_data['name'],
                value=user_data['value'],
                inline=True
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(silver_users)}**",
            inline=False
        )
        ...
        # Paginación para muchos usuarios
        embeds = create_paginated_embeds(silver_users, "💰 Pagos - Rol Silver", 0xC0C0C0, items_per_page=8)
...
@bot.tree.command(name="paga_expediente", description="Ver créditos de usuarios con rol Expediente")
async def paga_expediente(interaction: discord.Interaction):

    all_times = tracker.get_all_user_times()

    if not all_times:
        embed = discord.Embed(
            title="💰 Pagos Expediente",
            description="No hay usuarios con tiempo registrado.",
            color=0x1ABC9C
        )
        await interaction.response.send_message(embed=embed)
    ...
    # Recopilar usuarios con rol Expediente
    expediente_users = []
    total_credits_to_pay = 0
    ...
    if not expediente_users:
        embed = discord.Embed(
            title="💰 Pagos - Rol Expediente",
            description="No hay usuarios con rol Expediente registrados.",
            color=0x1ABC9C
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # Ordenar usuarios por créditos (mayor a menor)
    expediente_users.sort(key=lambda x: int(x['value'].split('**Créditos totales:** ')[1]), reverse=True)

    # Crear embeds paginados si hay muchos usuarios
    if len(expediente_users) <= 8:
        embed = discord.Embed(
            title="💰 Pagos - Rol Expediente",
            description=f"**{len(expediente_users)} usuarios con rol Expediente**",
            color=0x1ABC9C
        )
        
        for user_data in expediente_users:
            embed.add_field(
                name=user_data['name'],
                value=user_data['value'],
                inline=True
        ...
        embed.add_field(
            name="💎 RESUMEN TOTAL",
            value=f"**Total de créditos a pagar: {total_credits_to_pay}**\n**Total de usuarios: {len(expediente_users)}**",
            inline=False
        )
        ...
        # Paginación para muchos usuarios
        embeds = create_paginated_embeds(expediente_users, "💰 Pagos - Rol Expediente", 0x1ABC9C, items_per_page=8)
...
@bot.tree.command(name="dar_creditos", description="Dar créditos a usuarios con roles Alto, Supervisor, Silver o Expediente")
async def dar_creditos(interaction: discord.Interaction, usuario: discord.Member, cantidad: int):
    # Verificar que la cantidad sea válida (entre 1 y 100)
    if cantidad < 1 or cantidad > 100:
        embed = discord.Embed(
            title="❌ Error",
            description="La cantidad de créditos debe estar entre 1 y 100.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    if user_role not in allowed_roles:
        embed = discord.Embed(
            title="❌ Error",
            description=f"Solo se pueden dar créditos a usuarios con roles: Alto, Supervisor, Silver o Expediente.\n"
                       f"{usuario.mention} tiene rol: {user_role.title()}",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    success = add_credits_to_user(usuario.id, cantidad)
    ...
    if success:
        ...
        embed = discord.Embed(
            ...
            description=f"Se han otorgado **{cantidad} créditos** a {usuario.mention}\n"
                       f"**Rol:** {role_display}\n"
                       f"**Otorgado por:** {interaction.user.mention}",
            color=0x00ff00
        )
        
        # Obtener créditos totales del usuario
        total_credits = get_user_saved_credits(usuario.id)
        embed.add_field(
            name="💎 Total de créditos",
            value=f"{total_credits} créditos",
            inline=False
        )
        
        await interaction.response.send_message(embed=embed)

        # Notificar en canal de milestones
        milestone_channel = bot.get_channel(1382195219939852479)
        if milestone_channel:
            try:
                notificacion = f"💰 **Créditos otorgados manualmente:**\n"
                notificacion += f"{usuario.mention} recibió {cantidad} créditos - Cargo: {role_display}\n"
                notificacion += f"Otorgado por: {interaction.user.mention}"
                await milestone_channel.send(notificacion)
            except Exception as e:
                print(f"Error enviando notificación de créditos manuales: {e}")
    else:
        embed = discord.Embed(
            title="❌ Error",
            description="No se pudieron otorgar los créditos.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="restar_creditos_guardados", description="Restar créditos guardados a usuarios con roles Alto, Supervisor, Silver o Expediente")
async def restar_creditos_guardados(interaction: discord.Interaction, usuario: discord.Member, cantidad: int):
    # Verificar que la cantidad sea válida (entre 1 y 100)
    if cantidad < 1 or cantidad > 100:
        embed = discord.Embed(
            title="❌ Error",
    ...
    if user_role not in allowed_roles:
        embed = discord.Embed(
            title="❌ Error",
            description=f"Solo se pueden restar créditos a usuarios con roles: Alto, Supervisor, Silver o Expediente.\n"
                       f"{usuario.mention} tiene rol: {user_role.title()}",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    current_credits = get_user_saved_credits(usuario.id)
    
    # Verificar que el usuario tenga suficientes créditos
    if current_credits < cantidad:
        embed = discord.Embed(
            title="❌ Error",
            description=f"{usuario.mention} solo tiene {current_credits} créditos guardados.\n"
                       f"No se pueden restar {cantidad} créditos.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    # Restar créditos al usuario (usar cantidad negativa)
    success = add_credits_to_user(usuario.id, -cantidad)
    ...
    if success:
        ...
        embed = discord.Embed(
            title="💸 Créditos restados",
            description=f"Se han restado **{cantidad} créditos** a {usuario.mention}\n"
                       f"**Rol:** {role_display}\n"
                       f"**Restado por:** {interaction.user.mention}",
            color=0xff6b6b
        )
        
        # Obtener créditos totales del usuario después de la resta
        total_credits = get_user_saved_credits(usuario.id)
        embed.add_field(
            name="💎 Total de créditos restantes",
            value=f"{total_credits} créditos",
            inline=False
        )
        ...
        if milestone_channel:
            try:
                notificacion = f"💸 **Créditos restados manualmente:**\n"
                notificacion += f"{usuario.mention} perdió {cantidad} créditos - Cargo: {role_display}\n"
                notificacion += f"Restado por: {interaction.user.mention}\n"
                notificacion += f"Créditos restantes: {total_credits}"
                await milestone_channel.send(notificacion)
            except Exception as e:
                print(f"Error enviando notificación de créditos restados: {e}")
    else:
        embed = discord.Embed(
            title="❌ Error",
            description="No se pudieron restar los créditos.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def check_auto_start():
    ...
    try:
        ...
        if chile_time.hour == 22 and chile_time.minute == 00:
